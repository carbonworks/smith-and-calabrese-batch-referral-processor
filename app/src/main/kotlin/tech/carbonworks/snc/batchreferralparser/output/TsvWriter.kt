...
/**
 * Writes extracted referral data to TSV files using tab-delimited formatting.
 *
 * Produces a standard TSV file with the same column layout as the XLSX and CSV
 * exports. Fields are separated by tab characters. Embedded tabs, carriage
 * returns, and newlines are stripped from field values (no quoting is used).
 *
 * Column order is determined by the [ExportColumnConfig] passed to [write].
 * Date fields are written as plain text strings.
 */
object TsvWriter {
     ...
     *
     * Strips tab characters, carriage returns, and newlines from the value.
     * TSV does not use quoting — problematic characters are simply removed.
     *
