...
class PdfTextExtractor {
    ...
    fun extract(file: File): ExtractionResult {
        if (!file.exists()) {
            println("[PdfTextExtractor] File not found")
        ...
        println("[PdfTextExtractor] Opening file (${file.length()} bytes)")
        return try {
            Loader.loadPDF(file).use { document ->
                ...
                if (result is ExtractionResult.Success) {
                    ...
                    println("[PdfTextExtractor] Success: ${result.pages.size} page(s), $pagesWithText with text, $totalBlocks text block(s)")
                }
                result
            }
        } catch (e: InvalidPasswordException) {
            println("[PdfTextExtractor] Encrypted/password-protected: ${e::class.simpleName}")
            ...
            println("[PdfTextExtractor] I/O error: ${e::class.simpleName}")
            ExtractionResult.Error(
            ...
            println("[PdfTextExtractor] Unexpected error: ${e::class.simpleName}")
            ExtractionResult.Error(
