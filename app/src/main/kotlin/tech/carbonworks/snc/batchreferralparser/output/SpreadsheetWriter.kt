...
object SpreadsheetWriter {
     ...
     */
    val COLUMN_HEADINGS: List<String> = ExportColumnConfig.default().columns.map { it.displayName }
    ...
    fun write(
        ...
        // Filter to only enabled columns (including spacers)
        val activeColumns = columnConfig.columns.filter { it.enabled }
        ...
        XSSFWorkbook().use { workbook ->
            ...
            activeColumns.forEachIndexed { colIndex, column ->
                ...
                cell.setCellValue(if (column.isSpacer) "" else column.displayName)
            ...
            referrals.forEachIndexed { rowIndex, referral ->
                ...
                activeColumns.forEachIndexed { colIndex, column ->
                    ...
                    val fieldId = column.fieldId ?: return@forEachIndexed
                    val value = referral.getFieldValue(fieldId)

                    if (value.isNotEmpty()) {
                        if (fieldId in DATE_FIELD_IDS) {
